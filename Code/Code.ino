...
byte i2c_sensor_read_byte( int deviceaddress, int eeaddress ) {
 ...
 void setup() 
  {
    //char message[] = "enIts a lovely day today"; // data to write. N.B. language prefix 'en'
  ...
  void loop() 
  {
    ...
    // write temperature
//    String stringOne = "enTemperatures stored: ";
//    stringOne += (int) temperature;
//    char charBuf[stringOne.length()+1];
//    stringOne.toCharArray(charBuf, stringOne.length()+1);
//    i2c_eeprom_write_ndef(M24LR04, 4, (byte *)charBuf, sizeof(charBuf));
    
    // test max address file (40 characters allowed, not including the 'en' prefix)
//    char message[] = "enzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz401"; // data to write. N.B. language prefix 'en'
//    i2c_eeprom_write_ndef(M24LR04, 4, (byte *)message, sizeof(message));
    
    // Test new
    dataString += (int) temperature;
    dataString += ", ";
    char dataArray[dataString.length()+1];
    dataString.toCharArray(dataArray, dataString.length()+1);
    i2c_eeprom_write_ndef(M24LR04, 4, (byte *)dataArray, sizeof(dataArray));
    
    Serial.print("Current temperature is = "); Serial.print(temperature,DEC);
    Serial.println(" ");
